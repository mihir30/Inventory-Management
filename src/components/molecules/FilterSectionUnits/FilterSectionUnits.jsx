...
import './FilterSectionUnits.scss'
import InsertInvitationIcon from '@mui/icons-material/InsertInvitation'
import _moment from 'moment'
import 'twix'

const FilterSectionUnits = () => {
    ...
    const [condition, setCondition] = useState("Added");
    const [equipment, setEquipment] = useState("All units");
    const [month, setMonth] = useState("Month");
    const datePicker = useRef(null);
    const [iterator, setIterator] = useState("01-01-2020 - 02-01-2020");
    ...
    const handleClickLeftArrow = () => {
        ...
        switch (month) {
            case "Custom": {
                myDateStart.setDate(myDateStart.getDate() - (Math.random() * 10));
                myDateEnd.setDate(myDateEnd.getDate() - (Math.random() * 10));
                break;
            }
            case "Month": {
                myDateStart.setMonth(myDateStart.getMonth() - 1);
                myDateEnd.setMonth(myDateEnd.getMonth() - 1);
                break;
            }
            case "Year": {
    ...
    useEffect(() => {
        ...
        localStorage.setItem('status', JSON.stringify(condition));
        localStorage.setItem('type', JSON.stringify(equipment));
    }, [date, condition, equipment]);
    ...
    return (
        <div className='filter-section'>
            ...
            <div className='date-picker-box-units'>
            ...
            <div className='month-picker-box'>
                <Picker pickerType="form-select-month" aria-label="Month picker" value={month}
                    ...
                    <option selected value="Month" key="Month">Month</option>
                    <option value="Year" key="Year">Year</option>
                    <option value="12 months" key="12 months">12 months</option>
                </Picker>
            </div>

            <div className='condition-picker-box'>
                <Picker pickerType="form-select-condition" aria-label="Condition picker" value={condition}
                    onChange={(e) => setCondition(e.target.value)}>
                    <option selected value="Added" key="Added">Added</option>
                    <option value="Deprecated" key="Deprecated">Deprecated</option>
                    <option value="Broken" key="Broken">Broken</option>
                </Picker>
            </div>

            <div className='equipment-picker-box'>
                <Picker pickerType="form-select-equipment-units" aria-label="Equipment picker"
                    value={equipment}
                    onChange={(e) => setEquipment(e.target.value)}>
                    <option selected value="All units" key="All units">All units</option>
                    <option value="Computer" key="Computer">Computer</option>
...
export default FilterSectionUnits